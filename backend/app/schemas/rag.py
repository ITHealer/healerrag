...
def _validate_document_ids(value: list[int] | None) -> list[int] | None:
    if not value:
        return None
    invalid_ids = [document_id for document_id in value if document_id <= 0]
    if invalid_ids:
        raise ValueError("document_ids must contain positive document IDs")
    return value
...
class RAGQueryResponse(BaseModel):
    ...
    agentic_metadata: dict | None = None
...
class ChatSourceChunk(BaseModel):
    ...
    source_file: str = ""  # original filename for badge display (e.g. "NVIDIAAn.pdf")
