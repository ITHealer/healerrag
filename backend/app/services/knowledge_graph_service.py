...
async def _kg_llm_complete(
    ...
    **kwargs,
    ...
    # IMPORTANT: think=False is mandatory here.
    # LightRAG entity/relation extraction requires strictly structured output
    # (tuple format). When thinking mode is enabled, Ollama models output long
    # chain-of-thought reasoning instead of structured content, causing LightRAG
    # to log "LLM output format error" on every chunk and hang indefinitely.
    return await provider.acomplete(
        messages, temperature=0.0, max_tokens=4096, think=False,
    )
...
class KnowledgeGraphService:
    ...
    def __init__(
        ...
        self.kg_language = kg_language or settings.HEALERRAG_KG_LANGUAGE
        self.kg_entity_types = kg_entity_types or settings.HEALERRAG_KG_ENTITY_TYPES
        self._rag = None
    ...
    async def _get_rag(self):
        ...
        self._rag = LightRAG(
            ...
            chunk_token_size=settings.HEALERRAG_KG_CHUNK_TOKEN_SIZE,
    ...
    async def query(
        ...
        try:
            result = await asyncio.wait_for(
                ...
                timeout=settings.HEALERRAG_KG_QUERY_TIMEOUT,
            )
            return result or ""
        except asyncio.TimeoutError:
            logger.warning(
                f"KG query timed out after {settings.HEALERRAG_KG_QUERY_TIMEOUT}s "
