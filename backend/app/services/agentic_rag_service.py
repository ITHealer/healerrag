...
Orchestrator for the HEALERRAG pipeline:
...
class AgenticRAGService:
    """
    Full HEALERRAG pipeline orchestrator.
    ...
    def __init__(
        ...
        if settings.HEALERRAG_ENABLE_KG:
    ...
    async def process_document(self, document_id: int, file_path: str) -> int:
        """
        Process a document through the full HEALERRAG pipeline.
        ...
        try:
            ...
            # KG ingest (async, non-blocking failure, with timeout guard)
            if self.kg_service and parsed.markdown:
                import asyncio as _asyncio
                _kg_timeout = settings.HEALERRAG_PROCESSING_TIMEOUT_MINUTES * 60
                try:
                    await _asyncio.wait_for(
                        self.kg_service.ingest(parsed.markdown),
                        timeout=_kg_timeout,
                    )
                except _asyncio.TimeoutError:
                    logger.warning(
                        f"KG ingest timed out after {settings.HEALERRAG_PROCESSING_TIMEOUT_MINUTES}min "
                        f"for document {document_id}. Vector search still works without KG."
                    )
            ...
            logger.info(
                f"HEALERRAG processed document {document_id}: "
            ...
            logger.error(f"HEALERRAG failed for document {document_id}: {e}")
    ...
    async def delete_document(self, document_id: int) -> None:
        ...
        logger.info(f"Deleted document {document_id} from HEALERRAG stores")
