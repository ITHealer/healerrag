...
def _clean_chroma_where_fragment(value):
    """Remove empty Swagger placeholders and invalid empty filter fragments."""
    if value is None:
        return _DROP_WHERE
    ...
    if isinstance(value, dict):
        ...
        for key, item in value.items():
            if key == "additionalProp1" and item == {}:
                continue

            cleaned_item = _clean_chroma_where_fragment(item)
            if cleaned_item is _DROP_WHERE:
                continue
            cleaned[key] = cleaned_item

        return cleaned if cleaned else _DROP_WHERE

    if isinstance(value, list):
        cleaned_items = []
        for item in value:
            cleaned_item = _clean_chroma_where_fragment(item)
            if cleaned_item is not _DROP_WHERE:
                cleaned_items.append(cleaned_item)
        return cleaned_items if cleaned_items else _DROP_WHERE

    return value


def normalize_chroma_where(where: dict | None) -> dict | None:
    """Normalize app-level metadata filters to ChromaDB's one-root-key syntax."""
    cleaned = _clean_chroma_where_fragment(where)
    if cleaned is _DROP_WHERE or not isinstance(cleaned, dict):
        return None
...
class VectorStore:
    ...
    def query(
        ...
        where = normalize_chroma_where(where)
