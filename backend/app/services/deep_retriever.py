...
Pipeline:
  ...
  2. Vector search → over-fetch top-N candidates (HEALERRAG_VECTOR_PREFETCH)
  3. Cross-encoder rerank → precision filter to top-K (HEALERRAG_RERANKER_TOP_K)
...
class DeepRetriever:
    ...
    async def query(
        ...
        Flow:
          1. [parallel] KG query + Vector over-fetch (HEALERRAG_VECTOR_PREFETCH)
        ...
        prefetch_k = max(settings.HEALERRAG_VECTOR_PREFETCH, top_k * 3)
        ...
        if include_images and self.db and chunks:
            ...
            if page_nos:
                # Run sequentially — both use the same AsyncSession and SQLAlchemy
                # async sessions do not permit concurrent operations on the same session.
    ...
    def _rerank_chunks(
        ...
        reranked = self.reranker.rerank(
            ...
            documents=doc_texts,
            top_k=top_k,
            min_score=settings.HEALERRAG_MIN_RELEVANCE_SCORE,
        )
        ...
        if not reranked:
            ...
            logger.warning(
                ...
                f"{settings.HEALERRAG_MIN_RELEVANCE_SCORE}, falling back to top 3"
            )
