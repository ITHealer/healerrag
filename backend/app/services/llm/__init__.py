...
def get_llm_provider() -> LLMProvider:
    ...
    if provider == "gemini":
        ...
        if not api_key:
            raise ValueError("GEMINI_API_KEY or GOOGLE_AI_API_KEY is required when LLM_PROVIDER=gemini")
...
def get_embedding_provider() -> EmbeddingProvider:
    ...
    if provider == "gemini":
        ...
        api_key = settings.GOOGLE_AI_API_KEY or settings.GEMINI_API_KEY
        if not api_key:
            raise ValueError("GEMINI_API_KEY or GOOGLE_AI_API_KEY is required when KG_EMBEDDING_PROVIDER=gemini")
