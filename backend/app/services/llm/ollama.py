...
class OllamaLLMProvider(LLMProvider):
    ...
    def _extract_content(response, keep_thinking: bool = False) -> str | LLMResult:
        ...
        if not content and thinking:
            if keep_thinking:
                # Only use thinking-as-fallback when caller explicitly wants thinking output.
                # Do NOT use it for structured tasks (e.g. LightRAG KG extraction) where
                # thinking prose would break expected tuple-format output.
                logger.warning(
                ...
                logger.warning(
                    "Ollama response.content is empty (thinking=%d chars) — "
                    "returning empty string (think=False path)", len(thinking)
                )
