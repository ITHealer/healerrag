...
async def process_document_background(document_id: int, file_path: str, workspace_id: int):
    ...
    from app.core.database import async_session_marker
    from app.services.rag_service import get_rag_service

    async with async_session_marker() as db:
        try:
...
async def get_document_markdown(
    ...
    """Get the full structured markdown content of a document (HealerRAG parsed)."""
    result = await db.execute(select(Document).where(Document.id == document_id))
    document = result.scalar_one_or_none()
    ...
    if not document.markdown_content:
        raise HTTPException(
            ...
            detail="No markdown content available. Document may not have been processed with HealerRAG."
        )
