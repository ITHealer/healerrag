...
def _agentic_selected_chunks(state) -> list:
    if not state.context_budget:
        return state.merged_chunks
    selected_ids = set(state.context_budget.selected_chunk_ids)
    return [chunk for chunk in state.merged_chunks if chunk.chunk_id in selected_ids]


def _agentic_metadata(state) -> dict:
    from app.services.agentic.observability import state_observability_metadata

    metadata = state_observability_metadata(state)
    metadata.update({
        "enabled": True,
        "complexity": state.analysis.complexity.value if state.analysis else None,
        ...
        "remaining_items_count": (
            len(state.continuation_state.remaining_item_ids)
            if state.continuation_state
            else len(state.execution_plan.batch_later) if state.execution_plan else 0
        ),
        "selected_chunk_ids": state.context_budget.selected_chunk_ids if state.context_budget else [],
        "dropped_chunk_ids": state.context_budget.dropped_chunk_ids if state.context_budget else [],
        "response_judge_pass": state.response_judgment.pass_judge if state.response_judgment else None,
        "response_judge_issues": state.response_judgment.issues if state.response_judgment else [],
    })
    return metadata


def _agentic_chunks_to_chat_sources(state) -> list[ChatSourceChunk]:
    used_ids: set[str] = set()
    sources: list[ChatSourceChunk] = []
    for chunk in _agentic_selected_chunks(state):
        metadata = chunk.metadata or {}
        citation_id = _generate_citation_id(used_ids)
...
def _agentic_chunks_to_query_chunks(state) -> list[RetrievedChunkResponse]:
    ...
    for chunk in _agentic_selected_chunks(state):
        ...
        if isinstance(citation_data, dict):
            citation_resp = CitationResponse(
                ...
                document_id=int(citation_data.get("document_id", 0) or 0),
                page_no=int(citation_data.get("page_no", 0) or 0),
                heading_path=list(citation_data.get("heading_path", []) or []),
                formatted=str(citation_data.get("formatted", "")),
            )
...
def _agentic_image_refs(state, workspace_id: int) -> list[ChatImageRef]:
    images: list[ChatImageRef] = []
    seen: set[str] = set()
    used_ids: set[str] = set()
    for chunk in _agentic_selected_chunks(state):
        for image in (chunk.metadata or {}).get("result_image_refs", []) or []:
            image_id = str(image.get("image_id", ""))
            if not image_id or image_id in seen:
                continue
            seen.add(image_id)
            ref_id = _generate_citation_id(used_ids)
            used_ids.add(ref_id)
            images.append(
                ChatImageRef(
                    ref_id=ref_id,
                    image_id=image_id,
                    document_id=int(image.get("document_id", 0) or 0),
                    page_no=int(image.get("page_no", 0) or 0),
                    caption=str(image.get("caption", "")),
                    url=f"/static/doc-images/kb_{workspace_id}/images/{image_id}.png",
...
def _agentic_image_responses(state, workspace_id: int) -> list[DocumentImageResponse]:
    return [
        DocumentImageResponse(
        ...
        for image in _agentic_image_refs(state, workspace_id)
    ]


async def _run_agentic_orchestrator(
    *,
    query: str,
    workspace_id: int,
    db: AsyncSession,
    ...
    orchestrator = AgenticRAGOrchestrator.from_settings()
...
async def query_documents(
    ...
    if settings.AGENTIC_RAG_ENABLED and request.mode != "vector_only":
        try:
            state = await _run_agentic_orchestrator(
            ...
            chunks_response = _agentic_chunks_to_query_chunks(state)
            citations = [
                chunk.citation
            ...
            return RAGQueryResponse(
                ...
                citations=citations,
                image_refs=_agentic_image_responses(state, workspace_id),
                agentic_metadata=_agentic_metadata(state),
            )
        except Exception as exc:
            logger.warning("Agentic query failed, falling back to existing RAG query: %s", exc)
    ...
    if isinstance(rag_service, AgenticRAGService) and request.mode != "vector_only":
...
async def reindex_document(
    ...
    """Re-process an existing document through the HEALERRAG pipeline."""
    result = await db.execute(select(Document).where(Document.id == document_id))
    document = result.scalar_one_or_none()
    ...
    try:
        ...
        return DocumentProcessResponse(
            ...
            message=f"Re-indexed with HEALERRAG: {chunk_count} chunks created"
        )
    except Exception as e:
        raise HTTPException(
...
async def reindex_workspace(
    ...
    and re-processes every document through the HEALERRAG pipeline.
...
async def get_workspace_rag_stats(
    ...
    # Count HEALERRAG documents (parser_version = 'docling')
    agenticrag_result = await db.execute(
        select(func.count(Document.id)).where(
            Document.workspace_id == workspace_id,
            Document.parser_version == "docling"
        )
    )
    agenticrag_documents = agenticrag_result.scalar() or 0
    ...
    return ProjectRAGStatsResponse(
        ...
        agenticrag_documents=agenticrag_documents,
    )
...
async def get_workspace_analytics(
    ...
    # KG analytics (optional — only if HEALERRAG active)
    kg_analytics = None
    if agenticrag_documents > 0:
        try:
            kg = await _get_kg_service(workspace_id)
...
# ---------------------------------------------------------------------------
# Chat endpoint — LLM-powered document Q&A via HEALERRAG
# ---------------------------------------------------------------------------
...
async def chat_with_documents(
    ...
    """Chat with documents using HEALERRAG retrieval + LLM answer generation."""
    ...
    if settings.AGENTIC_RAG_ENABLED:
        try:
            ...
            sources = _agentic_chunks_to_chat_sources(state)
            image_refs = _agentic_image_refs(state, workspace_id)
            agentic_metadata = _agentic_metadata(state)
            try:
                import uuid
                from app.models.chat_message import ChatMessage as ChatMessageModel
                    ...
                    agent_steps=[
                        {
                            "id": "agentic-1",
                            "step": "agentic_rag",
                            "detail": f"Agentic RAG completed with {agentic_metadata['retrieval_attempts']} retrieval attempt(s)",
                            "status": "completed",
                            "timestamp": 0,
                            "metadata": agentic_metadata,
                        }
                    ],
                ))
                await db.commit()
            except Exception as persist_exc:
                logger.warning("Failed to persist agentic chat messages: %s", persist_exc)
                await db.rollback()
            return ChatResponse(
            ...
            logger.warning("Agentic chat failed, falling back to existing chat: %s", exc)

    rag_service = get_rag_service(db, workspace_id)

    # -- 1. Retrieve relevant chunks via HEALERRAG --
    chunks = []
    citations = []
    kg_summary = ""

    from app.services.agentic_rag_service import AgenticRAGService
    if isinstance(rag_service, AgenticRAGService):
        result = await rag_service.query_deep(
